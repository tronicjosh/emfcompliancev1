...
Radio-frequency antennas radiate electromagnetic energy in the form of time-varying electric and magnetic fields. In the far-field region, these fields propagate as electromagnetic waves and can be characterized using simplified relationships derived from Maxwell’s equations.

For safety assessment, the primary quantity of interest is the electric field strength 𝐸 (V/m) or equivalently the power density 𝑆 (W/m²).
...
Field magnitudes decay as 1/𝑟
Power density decays as 1/(𝑟^2)
...
θ,ϕ = angular coordinates relative to antenna orientation
...
where:
𝜂≈ 377Ω is the intrinsic impedance of free space

Rearranging:

E(r,θ,ϕ)= squareroot(η⋅S(r,θ,ϕ))
	​
...
𝑟 = square root((𝑥−𝑥0)^2+(𝑦−𝑦0)^2+(𝑧−𝑧0)^2)
Angular position relative to antenna boresight is determined.
Antenna gain G(θ,ϕ) is applied.
...
Compliance (𝑥,𝑦,𝑧)= {Safe, 𝐸 ≤ 𝐸limit
                    {Non-compliant  𝐸 > 𝐸limit
...
Exposure Index=∑Si/𝑆limit(𝑓i)


Compliance condition:

Exposure Index ≤1 ⇒ Compliant
...
Users do not define “fields” or “cells” directly — they define antennas, and the system computes exposure.
